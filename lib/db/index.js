...
class Db {
  ...
  /**
     ...
     * @param {string[]| number[] | string | []}  param     参数(可以是数组或者数字或者字符串,根据sql语句来定)
     * @param {string | null} mode    执行模式, 默认run,执行sql,如果查询的话,则使用get(单个)all(多个)
     * @returns {Promise}
       @used
  ...
  sql (sql, param, mode) {
    ...
    return new Promise((resolve, reject) => {
      if (!param) reject(new Error('no param'))
